...
router.post('/edit/(:id)', isLoggedIn, function(req, res, next) {
      ...
      db.collection('projects').find({
        "_id": ObjectId(o_id).toString
          ...
          if (req.user.local.role == "user") {
            ...
            if (!req.files.myFile) {
              finalCost = calcPrintCost(materialCost, project.fileVolumeCmCubed, req.body.projectDensity);
              ...
              db.collection('projects').updateOne({
                ...
                $set: {
                  ...
                  Density: req.body.projectDensity,
              ...
              if (theFile.data.length > 20000000) { // If file larger than x number of bytes, stop it from uploading
                ...
                theFile.mv(fullPath, function(err) {
                    ...
                    finalCost = calcPrintCost(materialCost, volume, req.body.projectDensity);
                    ...
                    }); //if the user DOES upload a new file
                    ...
                    var engineerInfo = JSON.parse(req.body.projEngineer);
                    var projectStatus = req.body.pro
    ...
    router.get("/quickUpdate/(:id)", isLoggedIn, function(req, res, next) {
      ...
      db.collection('projects').find({
        ...
        if (req.user.local.role == "engineer") {
          ...
          }  else if (project.status == "Printed") {
            console.log("Made it to printed")
            db.collection('projects').updateOne({
              "_id": project._id
            }, {
              $set: {
    ...
    function isRole(req, res, next) {
      if (req.isAuthenticated() && (req.user.local.role == 'admin' ||req.user.local.role == 'super'))
        return next();
